...
| `app/api/export`, `app/api/export/pdf` | The ZIP download (journal.json + photos) and the journal PDF, both start downloading straight away |
| `lib/fonts` | Noto Sans (SIL Open Font License, see `OFL.txt`), embedded in the PDF so names with ẹ, ọ, ṣ print correctly |
...
- **Dates** are calendar dates, stored as UTC midnight and shown in UTC so they never shift a day. "Today", the greeting and times like "expires 21 Sept, 15:08" use the visitor's own timezone, which a small script (`components/TimezoneCookie.tsx`) remembers in a `tz` cookie. Until it is known, `DEFAULT_TIMEZONE` is used (Africa/Lagos unless you set it).
...
- **Exporting**: "Download as PDF" builds the journal PDF on the server (jsPDF, with Noto Sans embedded) and downloads it in one click: entries as searchable text, photos underneath (JPG and PNG; WebP is skipped and the ZIP has it). "Download everything" streams a ZIP with `journal.json` and every photo, without holding the whole journal in memory.
...
- **Sign out on all devices**: every login carries the account's `sessionVersion`. Raising it (Profile > Sign out on all devices) cancels every other login. Pages, the photo route and the export all check it (`getCurrentUserId` in `lib/auth.ts`).
